...
class PanSmallRoundButton @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyle: Int = 0
) : FrameLayout(context, attrs, defStyle) {

    private val smallRoundButton by lazy { findViewById<AppCompatButton>(R.id.small_round_button)}
    private var buttonText: String? = null
    private var theme: Int = 0

    init {
        View.inflate(this.context, R.layout.pan_small_round_button, this)
        handleAttr(attrs)
    }
    ...
    fun setOnClickListener(action: () -> Unit) {
        smallRoundButton?.setOnClickListener { action.invoke() }
    }
    ...
    private fun getRoundedDrawable(color: Int) : ShapeDrawable {
        val radius = FloatArray(8) {
            resources.getDimension(R.dimen.dimen_16)
        }
    ...
    fun setTextButton(string: String?){
        string?.let {
            buttonText = it
            smallRoundButton?.text = buttonText
        }
    }
